...
def add_habit(habit_data):
    ...
    if 'target_unit' not in habit_data: habit_data['target_unit'] = 'times'
...
def get_projects(pending_only=True):
    ...
    df = pd.DataFrame(list(cursor))
    if df.empty: return pd.DataFrame(columns=['id', 'text', 'description', 'priority', 'is_completed', 'created_at'])
    df['id'] = df['_id'].astype(str)
    return df
