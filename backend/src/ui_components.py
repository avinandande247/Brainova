...
def render_badge_showcase(unlocked_badges):
    """Display all badges — unlocked ones in color, locked ones greyed out."""
    ...
    for i, (badge_id, badge_info) in enumerate(BADGES.items()):
        with cols[i]:
            ...
            if is_unlocked:
                st.markdown(
                    f"<div style='text-align:center; padding:15px; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border-radius:12px; border:1px solid #F63366;'>"
                    f"<span style='font-size:2rem;'>{badge_info['icon']}</span><br>"
                    f"<b style='color:#F63366;'>{badge_info['name']}</b><br>"
                    f"<small style='color:#aaa;'>{badge_info['desc']}</small>"
                    f"</div>",
                    unsafe_allow_html=True
                )
            else:
                st.markdown(
                    f"<div style='text-align:center; padding:15px; background:#1a1a2e; border-radius:12px; border:1px solid #333; opacity:0.5;'>"
                    f"<span style='font-size:2rem;'>🔒</span><br>"
                    f"<b style='color:#666;'>{badge_info['name']}</b><br>"
                    f"<small style='color:#555;'>{badge_info['desc']}</small>"
                    f"</div>",
                    unsafe_allow_html=True
                )


def render_today_progress_ring(completed, total):
    """Render a donut chart showing today's habit completion progress."""
    import plotly.graph_objects as go
    
    if total == 0:
        st.info("No habits due today.")
        return
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        hole=0.7,
        marker=dict(colors=['#F63366', '#262730']),
        textinfo='none',
...
def render_export_section():
    """Render data export buttons for habits and logs."""
    from src.data_manager import load_habits, load_logs
    
    st.markdown("### 📦 Export Your Data")
    
    c1, c2 = st.columns(2)
    with c1:
        habits = load_habits(active_only=False)
        if not habits.empty:
            csv = habits.to_csv(index=False)
            st.download_button(
                "📋 Download Habits (CSV)",
                csv,
                "habits_export.csv",
